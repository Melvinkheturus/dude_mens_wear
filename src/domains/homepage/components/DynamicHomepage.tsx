...
export default function DynamicHomepage() {
  ...
  const loadHomepageSections = async () => {
    try {
      setLoading(true)
      const homepageSections = await CollectionsService.getHomepageSections()
      ...
      const sectionsWithProducts = await Promise.all(
        homepageSections.map(async (section) => {
          ...
          const products = await CollectionsService.resolveCollectionProducts(section.collection)
          return { ...section, products }
        })
      )
