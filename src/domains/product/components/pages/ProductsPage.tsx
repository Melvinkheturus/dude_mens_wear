  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (category) {
          allProducts = await ProductsService.getProductsByCategory(category)
        } else if (query) {
          allProducts = await ProductsService.searchProducts(query)
        } else {
          allProducts = await ProductsService.getProducts()
        }

        // Always fetch new drops and bestsellers for homepage sections
        newArrivals = await ProductsService.getNewDrops(5)
        bestsellers = await ProductsService.getBestsellers(5)
