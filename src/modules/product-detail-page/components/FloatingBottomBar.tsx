...
export default function FloatingBottomBar({
  ...
  const { items, getTotalPrice } = useDemoCart()
  ...
  const displayPrice = price + getTotalPrice()
  
  // Check if customer can buy from floating bar
  const canBuyFromFloatingBar = items.length <= 1
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
        ...
        >
          <div
            className={`rounded-2xl bg-gradient-to-br from-red-600 via-red-500 to-red-700 shadow-2xl overflow-hidden ${
              isMobile ? '' : 'w-[900px]'
            }`}
            style={{
