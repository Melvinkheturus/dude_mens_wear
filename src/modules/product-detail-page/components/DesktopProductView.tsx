...
export default function DesktopProductView({ product }: DesktopProductViewProps) {
  ...
  useEffect(() => {
    setCurrentImage(product.images[selectedImage])
  ...
  // Demo data doesn't need variant IDs
  const getVariantId = () => {
    return undefined
  }
            ...
            <div className="absolute inset-0 p-6 md:p-8 lg:p-12">
              <div className="h-full flex flex-col lg:flex-row justify-between items-center gap-8">
                  ...
                  <div className="flex gap-3 mt-auto">
                    {product.images.slice(0, 4).map((img, idx) => (
                      <button
                        ...
                        className={`relative flex-shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-lg overflow-hidden border-2 transition-all bg-white hover:scale-105 ${
                          selectedImage === idx
                            ? 'border-black'
                ...
                <div className="w-full lg:w-72 bg-white/95 backdrop-blur-sm rounded-xl p-5 space-y-4 self-center relative">
                  ...
                  <ProductOptions
                    sizes={product.sizes}
                    colors={product.colors.map(color => ({ name: color, hex: '#000000', image: product.images[0] }))}
                    rating={4.5}
                    reviews={128}
                    selectedSize={selectedSize}
                    selectedColor={{ name: selectedColor, hex: '#000000', image: product.images[0] }}
