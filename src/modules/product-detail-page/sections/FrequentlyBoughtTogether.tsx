...
export default function FrequentlyBoughtTogether({
  ...
  const hasFBTItems = items.some(item => 
    products.slice(1).some(p => p.id === item.productId)
  )
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8">
              ...
              {products.map((product, index) => (
                <div key={product.id} className="flex items-center">
                  <motion.div
                    ...
                    className={`relative ${
                      product.selected ? 'opacity-100' : 'opacity-50'
                    }`}
                  >
                    ...
                    <button
                      ...
                      className={`absolute -top-1 -right-1 md:-top-2 md:-right-2 w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center font-bold text-xs md:text-sm transition-all ${
                        product.selected
            ...
            <div className="text-center lg:text-left w-full lg:w-auto lg:min-w-[280px]">
              ...
              <motion.button
                ...
                className={`w-full px-6 md:px-8 py-3 md:py-4 text-white text-sm md:text-base font-medium tracking-wide rounded-lg transition-all ${
                  isAdded || hasFBTItems ? 'bg-green-600 cursor-not-allowed' : 'bg-black hover:bg-gray-800'
                }`}
              >
