...
export default function ProductCard({ product, badge, badgeColor = "red" }: ProductCardProps) {
  ...
  const { addItem, items } = useDemoCart()
  ...
  const discountPercent = product.originalPrice ? Math.round(((originalPrice - currentPrice) / originalPrice) * 100) : 0
    ...
    <div className="group relative">
      ...
      >
        ...
        <div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-gray-50 transition-shadow duration-300 group-hover:shadow-xl">
          ...
          {/* Badge - Top Left */}
          {badge && (
            <span className={`absolute left-2 top-2 rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white md:left-3 md:top-3 md:px-2.5 md:py-1 md:text-xs ${
              badgeColor === "red" ? "bg-red-600" : "bg-black"
            }`}>
              {badge}
            </span>
          )}
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 flex items-center justify-between gap-1">
            ...
            <button
              ...
              className={`flex h-8 flex-shrink-0 items-center justify-center gap-2 overflow-hidden rounded-full transition-all ${
                isInCart
            ...
            >
              ...
              {isCartHovered && (
                <span className={`hidden whitespace-nowrap text-xs font-medium transition-colors md:inline ${
