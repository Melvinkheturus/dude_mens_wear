...
export default function OrderSummary({ isSticky = true }: OrderSummaryProps) {
  const { getTotalPrice, getTotalItems, getSubtotal, getShippingCost } = useDemoCart()
  const { user } = useAuth()
  ...
  const handlePincodeCheck = async () => {
    ...
    // Demo shipping calculation based on pincode
    const tamilNaduPincodes = ['600', '601', '602', '603', '604', '605']
    const isTamilNadu = tamilNaduPincodes.some(prefix => pincode.startsWith(prefix))
    
    setShippingFee(isTamilNadu ? 0 : 50) // Free shipping for Tamil Nadu
